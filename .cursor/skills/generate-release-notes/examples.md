...
[help/user-guide/release-notes.md](../../help/user-guide/release-notes.md)のペースト可能なパターン。 周囲のファイルに合わせてトーンと構造を調整します。

## 最小限のフロントマターの例（新しいページのみ）

新しいリリースノートのページを&#x200B;**作成**&#x200B;する場合にのみ使用します。 ファイルが既に存在する場合は、代わりにフルフロントマターを保持します。
...
このサブセクションは、ソース素材の修正または機能強化のラベルが&#x200B;**明示的**&#x200B;の項目にのみ使用してください。

## アーカイブされたブロック （以前のリリースノート）
