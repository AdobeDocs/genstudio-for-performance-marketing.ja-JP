...
左側の上にフィルター（ファネル）アイコンをクリックすると、**[!UICONTROL フィルター]** メニューが開き、複数のリストから選択できます。 一部のフィルターでは、正確なキーワードを活用して条件のリストを絞り込むことができます。 キーワードフィルターは、次のような複数の一意の識別子を持つ複雑な命名規則に従うキャンペーンで特に役立ちます。
