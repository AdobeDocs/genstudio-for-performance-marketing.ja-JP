...
>[!NOTE]
>
> このタスクを実行するには、Figma プラグインと [GenStudio for Performance Marketing用Photoshop プラグイン ](photoshop-plugin.md) の両方が必要です。

Figma プラグインを使用して、Figma フレーム、複数フレームまたはドキュメント全体をPhotoshop形式に変換し、[Photoshop プラグイン ](photoshop-plugin.md) で使用するために書き出すことができます。 現在、表示/非表示、フォントサイズ、基本レイヤー属性などの主要なプロパティのみが、変換時にサポートされます。 取り消し線、上付き文字、下付き文字、不透明度（割合）、グラデーションなどの機能は、まだサポートされていません。
...
1. 次のいずれかの操作を行います。
   ...
   * **[!UICONTROL Photoshop プラグインに転送]** をクリックして、変換されたファイルをキャッシュし、Photoshop プラグインですぐに使用できます。
     ![ 「Photoshop プラグインに転送」ボタン ](./transfer-to-ps-button.png){width="60%"}
1. **[!UICONTROL ファイルキーが必要]** ダイアログが表示されたら、プラグインは変換を完了するために Figma ファイル URL を必要とします。 ドキュメントの URL を追加します。

   1. Figma で、キャンバスの右上隅にある「**[!UICONTROL 共有]**」をクリックします。
   1. **[!UICONTROL このファイルを共有]** で、「**[!UICONTROL リンクをコピー]**」をクリックします。
   1. コピーしたリンクを、プラグインダイアログの「**[!UICONTROL Figma ファイル URL]**」フィールドに貼り付けます。

1. 「**[!UICONTROL 送信]**」をクリックします。 プラグインは、選択されたフレームを Figma で読み取り、JSON ドキュメントに変換します。JSON ドキュメントは、ファイルデータの仲介形式です。
   ![File Key Required ダイアログ ](./file-key-required.png){width="40%"}
1. Photoshopで、GenStudio プラグインを開いて「**[!UICONTROL 読み込み]**」タブをクリックします。
1. 次のいずれかの操作を行います。

   * 「**[!UICONTROL プラグインから]**」をクリックして、**[!UICONTROL Photoshop プラグインに転送]** で変換されたファイルを「キャッシュされたファイル」リストから選択します。または
   * 「**[!UICONTROL JSON をアップロード]**」をクリックし、アップロードする JSON ファイルを参照して選択します。
     ![ 「JSON をアップロード」ボタン ](./upload-json.png){width="40%"}
1. PhotoshopのGenStudio プラグインは、JSON ドキュメントの情報を、開いているPhotoshop ドキュメントに変換します。
