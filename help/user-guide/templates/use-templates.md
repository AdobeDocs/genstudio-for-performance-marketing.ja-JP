...
![&#x200B; コンテンツテンプレートリスト &#x200B;](/help/assets/content-templates-filter.png "LinkedIn テンプレートの検索"){width="650" zoomable="yes"}

テンプレート検索機能は、所有メディアまたは有料メディア用のテンプレートを選択する際に[!UICONTROL 作成]中に利用できます。 特定のフィルターオプションが表示されない場合は、リポジトリ内のテンプレートが対応するメタデータ条件に一致していないことを示します。 これらのフィルターを通じて検索できるように、テンプレートにメタデータが適切にタグ付けされていることを確認します。
...
1. _[!DNL Content]_&#x200B;で、上部バーの&#x200B;**[!UICONTROL HTML テンプレート]**&#x200B;セクションを選択します。

1. 「**[!UICONTROL + テンプレートを追加]**」をクリックします。

1. _[!UICONTROL 承認済みテンプレートを追加]_ ペインで、テンプレートタイプを選択します。 次に、HTML テンプレートファイルを参照するか、HTML テンプレートファイルをドロップスペースにドラッグします。 「**[!UICONTROL 次へ]**」をクリックします。

   ![&#x200B; ペイン内のテンプレートの種類](/help/assets/template-choose-type.png){width="500" zoomable="yes"}

1. _[!UICONTROL 検出されたフィールドを確認]_ ペインで、フィールドを確認します。 正しいテンプレートを使用していること、およびすべての詳細が期待どおりに表示されていることを確認します。

   >[!TIP]
   >
   >テンプレートの詳細が正しくない場合は、**[!UICONTROL 戻る]**&#x200B;をクリックして、前の手順に戻ります。 修正したテンプレートファイルをアップロードします。 または、[&#x200B; テンプレートコードエディター](/help/user-guide/templates/code-editor.md)を使用して簡単に修正できます。

1. 自動検出されたフィールドを確認します。 2つのパネルのプレビュー/コードビューでは、左側のパネルに、カラーで強調表示されたゾーンを含むテンプレートのライブプレビューが表示されます。右側のサイドバーには、現在のスロット状態で検出されたすべてのフィールドのフィールドリストが表示されます。 フィールドには、「コード」タブに表示されるハンドルバーの表記法（例：`{{headline}}`、`{{body}}`、`{{image}}`）でタグ付けされます。

   メールテンプレートのプレビューの例：

   ![&#x200B; プレビューフィールドが検出されました](/help/assets/template-detected-fields.png){width="650" zoomable="yes"}

1. 領域が自動検出されなかった場合は、プレビューパネルでその領域にカーソルを合わせると、**タグにホバー**&#x200B;機能が表示されます。 クリックしてハンドルバータグを割り当て、正しいスロット状態を設定します。

   ![&#x200B; ロゴフィールドのスロット状態を表示するホバーからタグへの機能](/help/assets/template-slot-state.png){width="500" zoomable="yes"}

   >[!TIP]
   >
   > **スロットの状態**
   >
   > 検出された各フィールドに対して、適切なスロットステートを割り当てます。
   >
   > | **スロットの状態** | **説明** | **&#x200B;**&#x200B;に使用 |
   > |---|---|---|
   > | **その他（編集可能）** | AIは、実行時にこのフィールドのコンテンツを生成するか、カスタムフィールドの手動編集を許可します。 | 見出し、本文、CTAなど、AIのバリエーションが必要なあらゆる場所。 手作業での編集やコンテンツフラグメントのスワップが可能なカスタムフィールドもあります。 |
   > | **修正済み** | AIまたはユーザーがフィールドを変更することはできません。 | 免責事項、規制の脚注、配信停止リンク。 |

1. または、「コード」タブでHTMLを手動で編集し、「**[!UICONTROL フィールドを自動検出]**」ボタンをクリックして、検出を再実行し、フィールドリストを更新することもできます。

1. テンプレートのプレビューに問題がなければ、**[!UICONTROL 次へ]**&#x200B;をクリックします。

1. _[!UICONTROL テンプレートの詳細を入力してアップロード]_&#x200B;で、テンプレートに名前を付け、**[!UICONTROL チャネル]** タイプを選択します。
...
1. 検索とフィルタリングでテンプレート識別を改善するために、できる限り多くの詳細を追加します。
