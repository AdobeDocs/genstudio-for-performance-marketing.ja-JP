...
Horizon キャンバスは、複数サイズまたは複数ページテンプレートのサイズとページ全体で関連するテキストフィールドと画像フィールドをリンクするようになりました。そのため、ラストマイルの編集は1回だけ行う必要があります。 編集メニューの&#x200B;**[!UICONTROL 適用]**&#x200B;範囲（このサイズ、すべてのサイズ、またはすべてのコンセプト）を選択すると、編集が自動的に反映されます。必要に応じてフィールドのタグ付けを手動で調整するオプションも用意されています。
