...
Horizon キャンバスでは、複数サイズや複数ページのテンプレートで、サイズやページをまたいで関連するテキストフィールドや画像フィールドがリンクされるようになりました。そのため、ラストマイル編集は 1 回のみ行う必要があります。 編集メニューの&#x200B;**[!UICONTROL 適用先]**&#x200B;範囲（「このサイズ」、「すべてのサイズ」または「すべての概念」）を選択すると、編集が自動的に生成されます。必要に応じて、フィールドのタグ付けを手動で調整するオプションも用意されています。

### The Trade Desk へのディスプレイ広告の直接アクティブ化

マーケターは、[!DNL GenStudio for Performance Marketing] から既存の The Trade Desk キャンペーンに承認済みのディスプレイクリエイティブを直接公開できるようになりました。 API トークンを使用してアカウントを接続し、[!DNL Activate] または [!DNL Content] からアクティベーションを開始し、The Trade Desk へのディープリンクを使用してアクティベーションのステータスを追跡します。

### Horizon キャンバスでのコンテンツフラグメントのスワップ

Horizon キャンバスで、テキストブロックを、AEM コンテンツフラグメントライブラリから直接取得した事前承認済みで再利用可能なコンテンツフラグメントにスワップできるようになりました。 コンテンツフラグメントのスワップでは一括編集を操作するので、スワップは単一のサイズに適用することや、すべてのサイズと概念をまたいで一度に生成することができます。これにより、手動で再入力することなく、規制対象となるコピーを承認済みとして正確に維持できます。
...
[!DNL Create] は現在、Horizon キャンバスで、2 つのテキストボックスをリンクさせて 1 つのフレーズを両方のテキストボックスにまたいでフローする Adobe Express 機能であるテキストフローを適用しています。 生成されたコピーは、リンクされたボックスを 1 つの接続されたフレーズとして埋めるので、メッセージが 2 つのボックスをまたいで分割する広告デザインでは、手動でコピーや再配置を行うことなく、最終的に生成のためにテンプレート化できます。 詳しくは、[リンクされたテキストフローボックス](/help/user-guide/templates/express-templates.md#use-linked-text-flow-boxes)を参照してください。
...
* **[!DNL Content]** の「**[!UICONTROL 削除]**」ボタンでのキーボードフォーカスが、期待どおりに機能するようになりました。 以前は、ユーザーはこのボタンにキーボードでアクセスできませんでした。  <!-- GS-4065 -->
