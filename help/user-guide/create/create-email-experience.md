...
* **バリアント](#edit-images-in-the-canvas)**&#x200B;で画像を[編集するには、画像フィールドをクリックし、**[!UICONTROL 編集]** （鉛筆アイコン）をクリックして、カンバス内で画像を直接切り抜き、再配置または展開します。
...
カンバスを離れたり、外部のデザインツールを使用したりすることなく、画像を調整できます。 画像を切り抜いて再配置し、被写体に焦点を当てることも、生成拡張を使用して画像を元の境界を超えて拡張することもできます。 編集は、電子メールの下書きにのみ適用されます。 [!DNL Content]および接続されたアセットリポジトリ内のSource アセットは変更されません。

電子メールキャンバスの任意の画像フィールドから画像編集にアクセスします。 編集した画像は視覚的に表示されるので、元のアセットから変更した画像を追跡できます。

### 画像の切り抜きと再配置

画像を切り抜いて、目的の領域のみを使用します。 メールの切り抜きはフリーフォームで、アスペクト比の制限はありません。そのため、画像のどの部分がどの割合で表示されるかを完全に制御できます。

**画像を切り抜いて再配置するには**:

1. メールのドラフトで、画像フィールドにカーソルを合わせてクリックします。
1. **[!UICONTROL 編集]** （鉛筆アイコン）をクリックします。
1. 「**[!UICONTROL 切り抜き]**」をクリックします。
1. 切り抜き領域を調整し、画像を目的の位置にドラッグします。
1. 選択した画像を切り抜くには&#x200B;**[!UICONTROL 適用]**&#x200B;をクリックし、下書きのすべての画像フィールドに切り抜きを適用するには&#x200B;**[!UICONTROL すべての画像に適用]**&#x200B;をクリックします。

変更を保存せずに終了するには、**[!UICONTROL キャンセル]**&#x200B;をクリックします。

![切り抜きツールを使用した画像の編集ビューは、メールキャンバス内の画像でアクティブになっています](/help/assets/edit-image-crop-email.png){width="650" zoomable="yes"}

### 生成拡張で画像を拡大

生成拡張を使用して、生成AIで画像を元の境界を超えて拡張します。 メールの拡大は、固定のサイズや割合に制限されないため、必要なスペースに画像を収めることができます。
...
1. **[!UICONTROL 展開]**&#x200B;をクリックします。
   ...
   拡張されたキャンバスの&#x200B;_[!UICONTROL 画像を編集]_ ビューで画像が開き、[!DNL GenStudio for Performance Marketing]は拡張オプションを生成します。

1. 画像を移動して、拡大領域内に配置します。
1. 生成されたオプションを選択するか、**[!UICONTROL 詳細]** （プラスアイコン）をクリックして他のオプションを表示します。
1. **[!UICONTROL 適用]**&#x200B;をクリックして選択した画像に結果を適用するか、**[!UICONTROL すべての画像に適用]**&#x200B;してドラフトのすべての画像フィールドに適用します。

変更を保存せずに終了するには、**[!UICONTROL キャンセル]**&#x200B;をクリックします。

>[!NOTE]
>
>生成拡張の出力サイズは制限されないので、品質は非常に大きな拡張に対して異なる場合があります。 **[!UICONTROL 詳細]**&#x200B;をクリックして、結果がニーズを満たさない場合に追加のオプションを生成します。

![生成拡張の結果が選択され、画像を編集ビューで適用する準備ができました](/help/assets/generative-expand-email.png){width="650" zoomable="yes"}
