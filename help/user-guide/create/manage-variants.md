...
[ エクスペリエンスの作成 ](/help/user-guide/create/overview.md) 時に、生成されたバリアントのコンテンツとアセットを個別にまたはバッチで変更できます。 個々のマイクロレベルでバリアントを管理する機能により、生成されたコンテンツの各部分を明示的に管理できます。
...
1. セクションのテキストを再生成する場合は、再生成アイコンをクリックすると、その他のテキストオプションが表示されます。
...
1. バリアントのセットを生成したら、「CTA」テキストを表示しているボタンをクリックします。
1. 「CTA」ボタンのテキストを変更するには、「_コールトゥアクション_ リストで使用可能なオプションから選択します。
1. CTAのリンクを変更するには、「**[!UICONTROL リンク]**」をクリックします。
   1. 「_リンクを編集_」セクションに新しいリンクを入力または貼り付けるか、既存のリンクを修正して「**[!UICONTROL 適用]**」をクリックします。 「**[!UICONTROL リンクを削除]**」をクリックすると、新しいリンクを入力する前に既存のリンクを削除できます。
